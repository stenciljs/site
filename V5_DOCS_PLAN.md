...
- **Phase 2 — Tier-1 rewrites. IN PROGRESS.** The files everyone hits early or that are actively wrong.
  ...
  - **Recommended next step, not yet started:** apply that same review process to `getting-started/01-getting-started.md`. It was rewritten in one bulk pass earlier in Phase 2, before any of the styling.md lessons existed, and it's the single highest-stakes page on the site (the literal first thing a new user reads). Treat it with the same skepticism styling.md got - verify claims against source rather than trust the first draft, and check it against the saved feedback memories (no meta-commentary, no contrastive slop, no em dash, no restating-padding, introduce-before-using, verify-before-documenting).
