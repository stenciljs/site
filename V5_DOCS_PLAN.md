...
These aren't v5-caused, found incidentally during the audit:

- **`build-variables.md` is currently unreachable.** `docusaurus.config.js`'s custom `sidebarItemsGenerator` excludes it (`EXCLUDE_TOP_LEVEL_IDS = ['build-variables', 'telemetry']`) and, unlike `telemetry`, it's never re-added anywhere in `sidebars.js`. Zero inbound links found. Only reachable by guessing the URL or a search-index hit. Fix: give it a real category (see mapping above).
- **`core/_category_.json` and `testing/_category_.json` both declare `position: 9`.** Docusaurus breaks the tie alphabetically (Core before Testing), which "works" by accident and will silently reorder on any rename. Fix as part of the IA rewrite anyway.
...
- **Phase 1 — IA restructure.** Move/rename files and folders per §2's mapping table, fix `sidebars.js`/`_category_.json` (including the position-9 tie), without yet rewriting prose content. Gets the skeleton right before pouring content into it.
- **Phase 2 — Tier-1 rewrites.** The files everyone hits early or that are actively wrong: `getting-started/`, `components/component.md` + the encapsulation sweep, `config/extras.md`→`compat`, `output-targets/{loader-bundle,standalone,ssr}.md`.
