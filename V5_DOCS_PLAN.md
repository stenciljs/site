...
**Added 2026-08-26, from John directly (not surfaced by the audit), corrected/expanded after a follow-up clarification — read this version, not the edit summary in git log for the first pass:**

The footgun reputation belongs to **shadow DOM**, not the alternative to it — plenty of developers avoid web components generally because shadow DOM's isolation (style, focus, event retargeting) is more than they want or understand. Stencil's actual position is that shadow DOM is *one* option, not *the* option, and the docs currently don't say so. Today's only real mention of the alternative (`styling.md`'s Scoped CSS section) frames it defensively ("does not, however, prevent styles from the light DOM from seeping into your component") instead of as a deliberate, well-supported, first-class choice.

Terminology, precisely: **"non-shadow"** covers both `encapsulation: { type: 'scoped' }` and `encapsulation: { type: 'none' }`. Within non-shadow, there are now two ways to get real style scoping:
- `type: 'scoped'` — Stencil's own mechanism (data-attribute-based CSS scoping + light-DOM patches for slot-like behavior), broad browser compatibility.
- `type: 'none'` **paired with the native CSS [`@scope`](https://developer.mozilla.org/en-US/docs/Web/CSS/@scope) at-rule** — the new "evergreen" alternative for modern-browser-only projects: real scoping with zero Stencil runtime mechanism, in place of `scoped`. (Verified 2026-08-26: `@scope` reached Baseline "Newly available" as of Firefox 146, Dec 2025; also Chrome 118+, Safari 17.4+, Edge 118+. Real, current, but "newly available" — be honest that some users may still be on pre-146 Firefox rather than claiming universal support.)

**The bigger point, beyond shadow-vs-non-shadow:** Stencil v5 has a first-class story for more than one component archetype, and the docs should present this as a spectrum, not corner cases:
- **Traditional JS-driven components** — the classic model, any encapsulation type, behavior lives in the class.
- **CSS-only components** — a custom element that's just a styled tag, no meaningful JS logic.
- **Progressively-enhanced components** — light-DOM/slotted content that's already meaningful before JS loads, with JS enhancing rather than creating the experience.

The claim tying it together: **no matter which archetype you pick, JS or no JS, your components stay fully typed and documented** — decorator-driven static analysis (`docs-json`, `docs-readme`, the CEM, `types-output-target`) doesn't care about encapsulation or runtime behavior, it reads the `@Component` class either way. And v5's new style co-location tools are what make the "before JS runs" and "after JS runs" states easy to manage together instead of as separate concerns: `@Component`'s new `globalStyleUrl`/`globalStyle` fields co-locate document-level styles with the component (collected at build time, injected wherever `@import "stencil-globals"` appears in a global stylesheet — works for every encapsulation type), and the `@import "stencil-hydrate"` placeholder injects static FOUC-prevention CSS at build time (the pre-upgrade state) instead of relying on the loader's dynamic `<style>` tag. Both verified directly against `stenciljs/V5_PLANNING.md`'s New Features section.

**Not yet verified, needed before drafting anything authoritative:** the exact mechanics of `<slot>`-equivalent content projection on non-shadow (`scoped`/`none`) components — native `<slot>` rendering is fundamentally a shadow DOM feature, so light-DOM "slotting" for scoped/none components goes through Stencil's `lightDomPatches` runtime instead; need to read that implementation (or write a scratch test component) before asserting how it behaves, rather than assume it's a drop-in equivalent to real `<slot>`. Also whether "progressively-enhanced component" is a named, documented pattern anywhere in the v5 engineering work or purely an emergent combination of existing features (SSR/SSG + non-shadow + slots + `stencil-hydrate`) — current answer is "no dedicated feature found," treat it as the latter unless something turns up.

See the expanded content item in §5 for where this goes structurally. **Draft in progress 2026-08-26**: `components/styling.md` now has a `## Choosing an Encapsulation Strategy` overview (shadow/scoped/none comparison table, framed as three equal options) plus a full `## No Encapsulation` section (light-DOM slotting mechanics, the `@scope` pairing) and a promoted, rewritten `## Scoped CSS` section — verified against the real `applyLightDomPatches`/slot-polyfill runtime source (`packages/core/src/runtime/{dom-extras,slot-polyfill-utils}.ts`) before writing the slotting explanation: Stencil's own renderer handles light-DOM slot placement unconditionally as part of its normal render cycle; `compat.lightDomPatches` only patches DOM-mutation entry points (`appendChild`, `insertBefore`, etc.) so code *other than* Stencil's renderer — frameworks, direct DOM calls — stays consistent with it, matching John's own description ("mainly just a compat layer for frameworks"). Builds clean. **Not yet committed — John wants to review before it's finalized, given deep familiarity with the mechanism.** Still needed: the archetype-spectrum content (traditional/CSS-only/progressively-enhanced) and the `getting-started`/`concepts` thread-through pointers.

**Added 2026-08-26, second general steer from John:** Stencil v5's real-world usage and first-class resourcing is **design systems, not apps** (matches `V5_PLANNING.md`'s own "90%+ design systems, not apps" framing behind the `loader-bundle`-over-`www` default change). App-oriented features — `www`, pre-rendering/SSG, service workers, routing — remain fully supported but should get proportionally less emphasis across v5 docs: don't cut them, but don't give them equal billing with component-authoring/output-target/testing content either. Concretely:
- `output-targets/01-overview.md`'s rewrite already leans this way ("set `www` explicitly if you're building a full web app") — keep that framing consistent everywhere else `www` comes up.
- `getting-started/01-getting-started.md`'s rewrite already centers `loader-bundle` (grounded in the real scaffold template, which defaults to it) — no change needed there.
- Still needs adjusting: the `guides/` subgrouping proposal in §4.6 lumps `service-workers.md` into a "PWA/performance" cluster at equal weight to the "Core patterns" group — reframe that cluster as explicitly app-oriented/niche rather than equally prominent. `output-targets/ssr/` (SSG/SSR/prerendering) is already nested rather than top-level, which is directionally consistent — no structural change needed, just keep its prose proportionate when it gets its full rewrite. No existing routing content was found in the current docs to check against.
...
| `design-systems.md` | **REMOVED (2026-09-07)** | Was the heaviest marketing-filler offender in the audit ("robust and highly extensible," icons that "pop") - a sketch pass showed even a fully rewritten, fact-checked version only produced two solid paragraphs' worth of real content, not enough to carry a dedicated page. Folded into `concepts/01-overview.md`'s "Design Systems & Component Libraries" section as five tightened, verified bullets (encapsulation choice, `globalStyleUrl` co-location, the per-component dev-server preview, generated docs incl. `docs-agent-skill`, framework wrappers). File deleted; both legacy redirects (`/what-is-design-system`, `/stencil-for-design-systems`) repointed from `/docs/:version/design-systems` to `/docs/:version/introduction` in `vercel.json`. |
...
**Internal subgrouping for `guides/`** (currently one flat, unordered list of 16 files at equal visual weight — no `_category_.json` subgroups exist today): group into **Core patterns** (forms, extends, store, typed-components) · **Shipping & distribution** (assets, publishing, module-bundling, csp-nonce, tag-transformation) · **Tooling** (vs-code-debugging, storybook, style-guide) · **PWA/performance** (service-workers, workers, build-conditionals). (Rendering strategy content moves out to `output-targets/ssr/` + `concepts/`, per §2. The former **Strategy** cluster - `design-systems.md` alone - no longer exists; that file was removed, see §4.6 above.)
...
0. **"Shadow DOM is one option, not the only one" — a real explanation of non-shadow encapsulation and multiple component archetypes** (added 2026-08-26, John's direct request, corrected/expanded 2026-08-26 — see the Vision section above for the full, corrected framing; don't work from an earlier summary of this item). Not currently planned content anywhere else in this doc; the closest existing thing is `component.md`'s `encapsulation` reference section (rewritten this session, but purely dry option-reference, no guidance on when/why) and `styling.md`'s defensively-worded `Scoped CSS` section. Needs, roughly:
   - **Where:** primary home is `components/styling.md` — add a real `## No Encapsulation` section (there's currently a `## Shadow DOM` and `## Scoped CSS` top-level section each, but nothing dedicated to `type: 'none'`, even though it's the *default*). Cover: what "no encapsulation" actually means, a real comparison table (shadow vs. scoped vs. none) covering style isolation, slot-equivalent behavior, DOM inspectability, SSR/SSG cost, and runtime patch overhead (`lightDomPatches`) — and reframe `Scoped CSS` alongside it so shadow DOM isn't implicitly the "normal" choice with the other two as fallbacks.
   - **The `@scope` angle:** `type: 'none'` + native `@scope` as the modern-browser-only "evergreen" alternative to `type: 'scoped'` — real scoping, zero Stencil runtime mechanism. State the Baseline status honestly (see Vision section for the verified numbers) rather than claiming universal support.
   - **The archetype spectrum:** traditional JS-driven components, CSS-only components (styled tag, no meaningful logic), and progressively-enhanced components (meaningful light-DOM/slotted content before JS, enhanced after) — presented as equally first-class, not corner cases. Tie together with the "fully typed and documented regardless of JS/encapsulation choice" claim, and the v5 style co-location story (`globalStyleUrl`/`globalStyle` + `@import "stencil-globals"`/`"stencil-hydrate"`) that makes pre-upgrade and post-upgrade styles easy to manage together.
   - **Verify before drafting:** exact light-DOM slot-projection mechanics on `scoped`/`none` components (goes through `lightDomPatches`, not native `<slot>` — don't assume it's a drop-in equivalent without checking the implementation or testing it). Confirm whether "progressively-enhanced component" is a named pattern anywhere in the v5 engineering work, or purely an emergent combination worth describing as such.
   - **Thread through:** a short, decisive pointer (not a repeat of the full explanation) from `components/component.md`'s `encapsulation` section, from `getting-started/` (which encapsulation/archetype to reach for and why, right when a reader is picking one for the first time), and from `concepts/` once that category exists.
...
7. **`output-targets/{collection,types,global-style,assets}.md`** - newly first-class targets with no current doc home. `global-style` specifically needs to fully document the `inject` option (`'none'`/`'client'`/`'all'`, `'none'` is the actual v5 default per `BREAKING_CHANGES.md`'s "Global Styles & Assets Modernized" section - `components/styling.md`'s Global Styles section deliberately only name-drops that the option exists (John: explaining `inject` in full is "too much too soon" for that page), so this is currently the only place that detail will live once written. It should also absorb the full `:host()`-targeting walkthrough (with the button/card/input examples) that used to live in `components/styling.md`'s "Constructable Stylesheets" section - that page trimmed to one paragraph pointing here, since the whole example only makes sense once `inject` is explained, and this is genuinely documentation for what `inject: 'client'`/`'all'` produces.
...
**Retracted 2026-08-26: `playground-elements` is the wrong default tool here, per John.** Original reasoning (2026-08-25) treated this like Lit's docs problem - Lit is mostly a thin runtime over native web components, so "run the source in a sandboxed browser iframe" (what `playground-elements` does) is a reasonably faithful stand-in for "show what the reader wrote." Stencil is different in a way that matters: its whole pitch versus Lit is that it's a **compiler** - decorators, JSX, `styleUrl`/`globalStyleUrl`, `encapsulation` are all resolved and transformed at build time, not just executed as-authored. `playground-elements` can only execute pre-built code in a sandbox; it can't run Stencil's compiler, so every demo built on it would have to hide the compile step behind a black box - exactly the "magic" a compiler-first project most needs to make legible, not hide. John: "giving people the full context is imperative."

**Also confirmed while investigating alternatives: running Stencil's own compiler live, client-side, isn't available.** Old search hits (a stale `compiler-api.md` v3 doc, a years-old JSFiddle, in-source JSDoc on `transpile()`/`transpileSync()` claiming browser support) pointed at genuine but obsolete capability - "In Browser Compilation Support Removed" is a real, documented v4.0.0 breaking change (`BREAKING_CHANGES.md`: "Prior to Stencil v4.0.0, components could be compiled from TSX to JS in the browser... has been removed from Stencil. At this time, there is no replacement functionality."). I should have cross-checked this against v5.0.0-section context I'd already read earlier in this same session before treating the old signal as current - noted as a gap in the verification habit, not just a one-off miss.

**Two candidate directions, from John, neither decided - needs a technical POC before committing to either:**
1. **`@stencil/unplugin`** (the new v5 universal bundler plugin for Vite/Rollup/webpack/rspack/bun, see `V5_PLANNING.md`) **running inside a real in-browser Node/bundler runtime.** [StackBlitz WebContainers](https://developer.stackblitz.com/platform/api/javascript-sdk) (Node.js compiled to WASM, runs fully client-side, verified 2026-08-26 via WebSearch as a strong fit specifically for "Node-shaped" tooling like Vite) is the concretely-identified option - spin up a real Vite + `@stencil/unplugin` project, edit source, get a real recompile with real HMR, genuinely showing the actual transformation because it *is* the actual transformation. Heavier than a sandboxed iframe (a real, if fast-booting, Node environment) and couples to StackBlitz's own embed/IDE shape.
2. **A custom-built playground UI** using [Sandpack](https://sandpack.codesandbox.io/) (CodeSandbox's embeddable editor/preview framework, built for exactly this "build your own dev-tool UI" use case) as the interface layer, backed by a real in-browser Node runtime (CodeSandbox's own "Nodebox," or another WebContainer-class runtime) for actual compilation underneath. More engineering investment, but buys full control over the UI shape - e.g. a Svelte-REPL-style layout (source / compiled output / live preview panes side by side), which is the right mental model here since Svelte faces the identical "compiler, not just runtime" documentation problem and solves it exactly this way. Nodebox's current capabilities weren't verified beyond generic vendor-comparison sources during this session - genuinely needs hands-on investigation, not more reading.

**Still valid, unaffected by the above:**
- **"Open a full working project" CTAs** (e.g., after the `stencil init` walkthrough, or a complete framework-integration recipe): StackBlitz SDK embeds - this may end up serving both the "full project" and "inline example" cases with one tool if direction 1 above pans out.
- **`ssr-wasm` polyglot examples (PHP/Java/Go/Rust/...): do not attempt live-in-browser execution.** Neither WebContainers nor Sandpack/Nodebox can run non-Node runtimes. Plan for verified, tested static code samples per language plus a linked minimal starter repo - that's the honest, deliverable version of "live examples" for this family.

**First concrete use case (tool-agnostic), from John:** `components/styling.md`'s "Choosing an Encapsulation Strategy" section is the flagship candidate for whichever direction the POC lands on - three near-identical demo components differing only in `encapsulation` (`shadow`/`scoped`/`none`), same CSS selector applied to all three, shown live side by side so the reader *sees* the isolation difference instead of reading three parallel prose descriptions of it. The page already has an inline `<!-- TODO(live-demo): ... -->` HTML comment marking the exact insertion point (see `components/styling.md`, "Choosing an Encapsulation Strategy" section) - update that comment once a direction is chosen, it currently still names `playground-elements` specifically.

**Considered and rejected, 2026-08-26: a second live demo for global styling** (co-location via `globalStyleUrl`, `@import "stencil-globals"` merging multiple components' CSS, multiple `global-style` outputs). This is almost entirely a **build-time** transformation regardless of which tool gets chosen above for the encapsulation case - direction 1 (`@stencil/unplugin` in a real Node runtime) *could* actually show this live since it runs the real compiler, but the payoff is much smaller than the encapsulation case (multi-file CSS merging is less visually dramatic than live shadow-DOM isolation) and it's not worth scoping into the same POC. The one genuinely runtime-visible piece of the global-styles story (a global stylesheet's `:host(my-button)` rule reaching a live shadow root once `inject: 'client'` is set) is exactly the `inject` detail already deferred out of `components/styling.md` to the future `output-targets/global-style.md` page (§4.4/§7 item 1) - revisit whether it deserves a live demo once that page and the playground direction both exist.
...
- **Phase 2 — Tier-1 rewrites. IN PROGRESS.** The files everyone hits early or that are actively wrong.
  ...
  - **`components/styling.md` went through extensive further review after its initial encapsulation-sweep pass above (2026-08-26, still uncommitted as of this writing — check `git status` / `git diff HEAD -- docs/components/styling.md`).** John line-by-line reviewed the whole file and caught real issues the first pass missed: wrong technical claims (scoped CSS uses a generated *class*, not a data attribute; `lightDomPatches` applies to `type: 'none'` too, not just `scoped`; `global-style`'s default `dir` is the unified `dist/assets/`, not tied to `www`/`loader-bundle`; shadow DOM *is* visible in browser DevTools; `::part()`/CSS-only-components framing errors), writing-voice issues (meta-commentary rebutting a framing only I'd held, formulaic "X is a real Y, not Z" constructions, em dashes, restating-padding, naming a mechanism before its problem), structural issues (the whole page's Shadow DOM/Scoped CSS/No Encapsulation three-section layout was redundant and got collapsed into one section with condensed per-type notes; misplaced content; `globalStyle` config fully replaced by `global-style` output target as the taught mechanism), and missing content (the `@prop` JSDoc convention for documenting CSS custom properties, previously undocumented on this page). All nine resulting lessons are saved as feedback memories for this project - **read them before writing or reviewing any more docs prose here**, they're a real, current quality bar, not just this-conversation context.
  - **Done (2026-09-04):** applied that same review process to `getting-started/01-getting-started.md`. Two real bugs that would break for a live reader, both caught by verifying against source/a real build rather than trusting the first draft: `npm start` right after scaffolding (no `start` script exists in the real `component-starter` `package.json` - only `dev`/`build`/`generate`; fixed to `npm run dev`), and the CDN script-tag `src` path (missing the namespace subdirectory and wrong filename - confirmed by actually running `stencil build` against a hand-built scratch project and inspecting the real `dist/loader-bundle/` tree; fixed to the verified `<fsNamespace>/<fsNamespace>.js` pattern with a one-line explanation of the mapping). Also fixed: a backwards claim about `stencil init`'s add-capabilities mode vs. `stencil add` being "for a single package" (read `task-init.ts`/`task-add.ts` directly - both go through the identical multi-select `promptAddCapabilities` flow when run without arguments; the real distinguishing feature is `stencil add <package>`'s explicit-argument form, not a single-vs-multiple split); and a structural bug where "you have two main options" introduced two sections that were siblings instead of nested, with a summary sentence referencing npm link before it was introduced (renested to h4, moved the sentence). Plus the usual writing-quality sweep (em dashes, a restating-padding trim, a dropped-comma inconsistency). Two items came from John's live read: cut the automatic-JSX-runtime aside entirely (compiler internals a beginner doesn't need for the next step) and cut the "not yet supported in CI" note (irrelevant this early, and a hedge that goes stale) - both generalized into a new feedback memory (`tutorial-mode-cut-irrelevant-asides`, broadened from an initial narrower version) about deleting rather than trimming asides the immediate step doesn't need. Also added, per John: a jsDelivr-after-`npm publish` mention with a link to `guides/publishing.md`, and a `TODO(live-demo)` placeholder in "My First Component" matching the convention already in `styling.md` - this page is the strongest live-demo candidate on the site since it's the reader's first contact with the compile step. **Surfaced but deliberately not fixed:** `guides/publishing.md`'s "Lazy Loading" section has the same class of bug (a stale `unpkg` package.json field/path - v5 never generates that field, and the real path needs the namespace subdirectory) - left alone since that file is already a full REWRITE target (§4.6) and patching one phrase in isolation would leave the rest of its stale `dist`/`dist-custom-elements` prose untouched.
  - **Done (2026-09-07):** two follow-ups from a second read-through of `getting-started/01-getting-started.md`, both from John directly. (1) `guides/design-systems.md` removed entirely rather than rewritten - a sketch pass showed even a fully fact-checked version only produced two solid paragraphs' worth of real content, not enough to carry a dedicated page (see updated `design-systems.md` row above). Folded into `concepts/01-overview.md` as five tightened bullets instead. (2) The whole "Local Development" section of `getting-started/01-getting-started.md` removed: its "Framework Integration" subsection was a non-sequitur under a "local development" heading (just a redirect elsewhere), "Using Script Tags" recommended manually copying `dist/loader-bundle/` into a consuming app on every change (bad DX, nobody would actually do this), and "Using npm link" explained a generic npm mechanism out of this site's scope. Replaced with a short "Next Steps" section, corrected once during review: publishing (`guides/publishing.md`) is the universal step for getting components into another project either way, not an alternative to framework integration - `framework-integration/01-overview.md` generates optional framework-native wrapper packages *on top of* a published library, confirmed by checking that `framework-integration/react.md` itself never runs a real `npm publish`, only local monorepo linking, and sets `publishConfig` assuming a separate publish step. Verified with a full production build both times (no broken links).
  - **Done (2026-09-10):** `guides/publishing.md` fully rewritten (this was flagged as higher-priority after the getting-started work above linked to it directly). Retitled to `Publishing & Consuming a Component Library` (slug kept at `/publishing` deliberately - no redirect needed, but see the slug-vs-title note in memory if this comes up again) since the content is genuinely both. Restructured publish-then-consume instead of the old consume-then-publish-as-an-afterthought order; real numbered publish steps added (there were none before, just a link to npm's own docs), including a genuinely new-to-the-audit gotcha: the default `component-starter` template's `.gitignore` excludes `dist/`, so a naive `npm publish` silently ships a package missing its entire build output unless `files`/`.npmignore` is set - confirmed by reading the template's `_gitignore` directly. Fixed several stale/wrong technical claims caught by verifying against real builds, not the first draft: the CDN script-tag path (missing the namespace subdirectory, same bug class as the getting-started fix); a false claim that a bare `import 'pkg'` auto-registers loader-bundle components (it doesn't - resolves to an empty placeholder unless `generateExportMaps` or an explicit `exports` field points it at the loader); the standalone root-import example claiming `defineCustomElementMyComponent` is the export name (it's just `defineCustomElement` by default - the suffixed form only appears under `customElementsExportBehavior: 'single-export-module'`); a wrong per-component `types` path; a fabricated helper-export list; two invalid JSON trailing commas; and a flat contradiction between prose ("standalone does not define the custom elements") and the code example directly below it (which calls `defineCustomElement()` to do exactly that).
  - **Three real engineering fixes landed mid-session in response to these doc findings** (all re-verified against source + a real build before being written up, per the `engineering-fixes-reverify` memory - a paraphrase of "fixed" is not the same as the actual resulting mechanics): `927c909e9` gave `loader-bundle`/`standalone` a `generateExportMaps` config option (real default: `true` only when there's no `stencil.config.ts` at all - a wizard-scaffolded project still needs it set explicitly, since `stencil init` always writes a config file... except `374026fa1` then fixed a genuine bug where the static `component-starter` template shipped its own `stencil.config.ts` unconditionally, defeating the wizard's own "zero-config (loader default)" status message - now truly zero-config for default selections). `f968c9d3f` + `265ad5465` added a build-time warning when `package.json` is missing a `files` array covering all distributable output dirs (not just `collection`, after a first pass that only checked that one). `6f6ac8c97` + `aaf0ebddd` made `standalone`'s asset-path auto-configuration symmetric with `loader-bundle`'s (previously only root/loader entries auto-called `setAssetPath` at build time; now every per-component subpath does too, with `setAssetPath` exported everywhere as an override for bundler-relocation cases).
  - **Also done:** `output-targets/01-overview.md` gained a new "Choosing Between `loader-bundle` and `standalone`" section (the CMS-vs-static-bundler framing for when each is the right call, plus a real, verified note on why sequential nested-dependency loading isn't as bad as it sounds - `generateComponentBundles`/`optimizeBundlers` in `compiler/entries/component-bundles.ts`, called from `dist-lazy/lazy-output.ts`, groups frequently-co-occurring components into shared bundles by a similarity score, reducing but not eliminating the sequential-load count). This absorbed the "Considerations" content that used to live in `publishing.md` (Diátaxis split: "which output target and why" is explanation-mode, doesn't belong in a publishing how-to).
  - **Surfaced, not yet fixed:** `output-targets/custom-elements.md`'s "Making Assets Available" section (lines ~140-165) is now stale in the same way `publishing.md` was before this session's fixes - it still frames `standalone` asset paths as requiring a manual `setAssetPath()` call, with an example using a raw deep import (`my-library/dist/standalone`) instead of the exports-map subpath pattern established everywhere else. The rest of that page (`autoLoader`, `customElementsExportBehavior`'s four real values) is already accurate - this is a narrowly-scoped fix, not a full-page rewrite.
  - **Recommended next content target: `guides/extends.md`.** Flagged earlier in the audit as "actively incorrect for v5, high priority" and never actually started this session (got sidetracked into the publishing/consuming work above instead) - it's the single highest-severity unfixed item on the whole site: not just stale, but actively wrong. It documents `class MyComponent extends ReactiveControllerHost` directly, which the plan says breaks under `standalone` builds (the class-extension AST rewrite can't touch prebuilt `node_modules` code) - the real v5 pattern is `class MyComponent extends Mixin(ReactiveControllerHost)`. A reader would copy-paste the current example and get broken code.
  - **Also not yet done:** the `ssr` output target rewrite/merge; `output-targets/{collection,types,global-style,assets,docs-agent-skill}.md` (net-new, §5); `framework-integration/*` rewrites; `guides/module-bundling.md`.
