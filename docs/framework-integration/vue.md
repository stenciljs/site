...
By default v-model value is taken from event's `detail.${targetAttr}` property. This behaviour can be overridden by setting `eventAttr` property on the `ComponentModelConfig`:
```tsx
