...
Introduced in [Stencil v2.18.0](https://github.com/ionic-team/stencil/releases/tag/v2.18.0), `transformAliasedImportPathsInCollection` was a configuration flag on the [`dist` output target](../output-targets/loader-bundle.md) (as of Stencil v5, this same functionality lives on the `collection` output target's `transformAliasedImportPaths` option, since collection output is now its own first-class output target rather than a sub-output of `dist`).
...
For more information on this flag as it existed in v4, please see the [`dist` (now `loader-bundle`) output target's documentation](../output-targets/loader-bundle.md).
...
For more context on this change, see the [documentation for the new `supplementalPublicTypes`](../output-targets/documentation-generation/docs-json.md#supplementalpublictypes)
